...
----------------------------------------- ВНИМАНИЕ! ГОВНОКОД! --------------------------------------------------
    ...
    if show_setting_menu.v then 
        imgui.SetNextWindowPos(imgui.ImVec2(400,460), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
        imgui.SetNextWindowSize(imgui.ImVec2((280), 170), imgui.Cond.FirstUseEver)
    ...
    if checkUpdate.v then
        ...
        if imgui.Button(u8'Скрипт', btn_size) then 
            local update_url = 'https://raw.githubusercontent.com/yummyme/plist/master/resource/versions.ini'
